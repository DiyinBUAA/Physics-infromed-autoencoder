...
class My_loss(nn.Module):
    ...
    def greater_than_zero(self,x):
        return (x >= 0).float()

    def forward(self, outputs1, targets1,outputs2, targets2, hiddenstate1, hiddenstate2, log_sigma_u, log_sigma_f, log_sigma_f_t):

        loss_prior = torch.sum((outputs1 - targets1) ** 2)/outputs1.shape[0]

        loss_poster =  torch.sum((torch.max(abs(outputs2),torch.tensor(1.0))-torch.tensor(1.0))** 2)/outputs2.shape[0]

        loss_hidden = torch.sum(self.greater_than_zero(targets2-outputs2)** 2)/outputs2.shape[0]
        if self.mode == 'Baseline':
            loss =loss_prior
        if self.mode == 'Sum':
            loss = loss_prior +10**5*loss_poster+10**2*loss_hidden
        if self.mode == 'AdpBal':
            loss = torch.exp(-log_sigma_u) * loss_prior + torch.exp(-log_sigma_f) * loss_poster+ torch.exp(-log_sigma_f_t) * loss_hidden + \
                   log_sigma_u + log_sigma_f + log_sigma_f_t

        self.loss_U = loss_prior
        self.loss_F = loss_poster
        return loss

def get_batterynumber(testid):
    mapping = {
        1: 'CS2_33',
        2: 'CS2_34',
        3: 'CS2_35',
    ...
    return mapping.get(testid, 'Invalid testid')